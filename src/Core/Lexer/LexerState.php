...
final class LexerState
{
    ...
    {
        return $this->modes[count($this->modes) - 1];
    }

    public function pushMode(string $m): void
    {
        $this->modes[] = $m;
    }
    ...
    {
        if (count($this->modes) > 1) array_pop($this->modes);
    }

    public function len(): int
    {
        return strlen($this->src->code);
    }
